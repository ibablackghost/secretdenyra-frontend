...
export const Cart = () => {
  ...
  useReconcileCartWhenReady(true);
  ...
  if (items.length === 0 || (!loading && products.length > 0 && cartProducts.length === 0)) {
    return (
      <EmptyState
        ...
        description={
          staleLineCount > 0
            ? 'Des articles enregistrés sur cet appareil ne sont plus disponibles. Videz le panier et rajoutez vos produits.'
    ...
    <div className="max-w-[1400px] mx-auto px-4 md:px-8 py-12 md:py-20">
      ...
      {staleLineCount > 0 ? (
        <div className="mb-6 rounded-[12px] border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
          <p className="font-semibold">Articles introuvables dans le catalogue</p>
          <p className="mt-1">
            {staleLineCount} ligne(s) obsolète(s). Videz le panier puis rajoutez vos produits depuis la boutique.
          </p>
          <button type="button" onClick={() => void clearCart()} className="mt-3 font-semibold text-[#a4a374] underline">
            Vider le panier
          </button>
        </div>
      ) : null}
